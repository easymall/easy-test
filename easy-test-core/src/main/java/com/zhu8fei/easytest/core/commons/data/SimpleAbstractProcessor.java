...
import com.zhu8fei.easytest.core.commons.annotation.DataSetAnnotationUtils;
import com.zhu8fei.easytest.core.commons.bean.DataSetBean;
import com.zhu8fei.easytest.core.commons.bean.ExpectBean;
import com.zhu8fei.easytest.core.commons.bean.PrepareBean;
 ...
 */
public abstract class SimpleAbstractProcessor implements DataProcessor {
    private Logger logger = LoggerFactory.getLogger(SimpleAbstractProcessor.class);

    private DataBaseMapper dataBaseMapper;

    @Override
    public void insertPrepareData(Method method) throws EasyTestException {
    ...
    @Override
    public boolean compareResult(Method method) throws EasyTestException {
        boolean isLog = DataSetAnnotationUtils.isLog(method);
        // 读取数据
        String context = DataSetAnnotationUtils.dataContext(method);
        if (isLog) {
    ...
    private boolean expectData(DataSetBean bean) {
        ...
        for (ExpectBean expectBean : expectBeans) {
            ...
            st.addAllColumns(columns);
            List<Map<String, String>> dataResult = dataBaseMapper.select(st);
            ...
            List<Map<String, String>> diff = target.stream()
                    // 过滤当前元素如果存在 则不拿取
                    .filter(x -> dataResult.stream().noneMatch(y -> y.equals(x)))
                    // 返回过滤后的集合
